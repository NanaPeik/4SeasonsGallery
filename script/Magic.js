$(document).ready(function(){
    ...
    const cars=["./moving/Img_05.png","./moving/Img_06.png"]
...
var c = document.getElementById("CanvasFirework");
...
function Run() {
  ...
  if (lastRun != 0) { dt = Math.min(50, (performance.now() - lastRun)); }
	...
	cont.fillStyle = "rgba(0,0,0,0.25)";
  ...
  for (let ix in shells) {
    ...
    cont.beginPath();
    cont.arc(shell.x * cwidth, shell.y * cheight, shell.size, 0, 2 * Math.PI);
    cont.fillStyle = shell.color;
    cont.fill();
  ...
  for (let ix in pass) {
    ...
    cont.beginPath();
    cont.arc(pas.x, pas.y, pas.size, 0, 2 * Math.PI);
    cont.fillStyle = pas.color;
    cont.fill();
